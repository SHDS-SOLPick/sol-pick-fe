...
const AllergyManagement = () => {
  ...
  const fetchAllergies = async () => {
    try {
      const response = await axios.get(`http://localhost:8090/api/user-allergy/${userId}`);
  ...
  const addAllergy = async () => {
    ...
    try {
        ...
        const response = await axios.post(
            `http://localhost:8090/api/user-allergy/${userId}?ingredientName=${encodeURIComponent(newAllergy)}`,
            {}, 
        ...
        console.error("❌ 알러지 추가 실패:", error.response?.data || error.message);
  ...
  return (
    <div className="allergy-container">
      ...
      <div className="input-group">
        ...
        />
        <button className="add-btn" onClick={addAllergy}>추가</button>
      </div>
        ...
        {allergies.length === 0 ? (
          ...
          allergies.map((allergy, index) => (
            <li key={index} className="allergy-item">
              {allergy}
              <button className="delete-btn" onClick={() => removeAllergy(allergy)}>❌</button>
            </li>
          ))
        )}
      </ul>
