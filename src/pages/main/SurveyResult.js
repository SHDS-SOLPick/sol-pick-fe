...
const SurveyResult = () => {
  ...
  const fetchMealPlan = async () => {
    try {
      ...
      const response = await axios.post("http://localhost:8090/api/meal-plan", formData);
      ...
      const formattedMealPlan = Object.entries(response.data).flatMap(([day, meals]) => {
        return Array.isArray(meals) ? meals.map(meal => ({ day, ...meal })) : []; // ✅ meals가 배열인지 확인
      });
  ...
  return (
    <div className="survey-result-container">
      ...
      {loading ? (
        <p className="loading">사용자님의 권장 영양소를 AI가 분석 중입니다...</p>
      ) : error ? (
          ...
          {mealPlan.reduce((acc, meal) => {
            ...
            return (
              <div key={index} className="day-card">
                <h3>
                  📅 {dayData.day} 
                  <span className="total-calories">총 {totalCalories} kcal</span>
                </h3>
                <div className="meals">
