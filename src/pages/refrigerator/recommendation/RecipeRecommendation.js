...
const RecipeRecommendation = () => {
  ...
  const fetchRecipeImage = async (recipeName) => {
    try {
      ...
      const response = await axios.get(`https://api.unsplash.com/search/photos`, {
        ...
        headers: { Authorization: `Client-ID EInmdWllYzZDqaUzMPpwDdWzIESwNeM2AfQwbs8nuK8` },
      });
  ...
  const fetchRecommendedRecipes = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:8090/api/refrigerator/recommend/${userId}`);
      ...
      const recipeList = await Promise.all(recipeData.recipes.map(async (recipe, index) => ({
        id: index + 1,
        name: recipe.name || "이름 없음",
        ingredients: recipe.ingredients ? recipe.ingredients.join(", ") : "재료 정보 없음",
        cooking_time: recipe.cooking_time || "조리 시간 정보 없음",
        difficulty: recipe.difficulty || "난이도 정보 없음",
        steps: recipe.steps || ["조리 방법 정보 없음"],
        image: await fetchRecipeImage(recipe.name) || "/images/default_recipe.jpg",
      })));

      setRecipes(recipeList);
      sessionStorage.setItem("recommendedRecipes", JSON.stringify(recipeList)); // ✅ 저장
    } catch (err) {
      setError(err.response?.data?.message || err.message || "네트워크 오류 발생");
    } finally {
      setLoading(false);
    }
  };
