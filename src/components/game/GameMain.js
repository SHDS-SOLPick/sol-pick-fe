...
const GameMain = ({ onDailyGame, onStorage }) => {
  ...
  useEffect(() => {
    const loadGameData = async () => {
      try {
        ...
        // 2. 선택한 레시피 정보 로드
        const recipeId = getSelectedRecipe(); // localStorage에서 로드
        if (recipeId) {
          const recipe = recipes.find((r) => r.id === recipeId);
          setSelectedRecipe(recipe);

          // 3. 발견한 식재료 정보 로드
          const discoveredIngredientsData = await getDiscoveredIngredients(
            recipeId
          );
          ...
          if (recipe) {
            ...
            const discoveredCount = discoveredIngredientsData.filter(
              (item) => item.discovered
            ).length;

            // 레시피 정보와 발견한 식재료 정보 결합
            const ingredientsWithIcons = recipe.ingredients.map((recipeIng) => {
              const serverIng = discoveredIngredientsData.find(
                (item) => item.ingredientName === recipeIng.name
              );

              return {
                name: recipeIng.name,
                icon: recipeIng.icon,
                discovered: serverIng ? serverIng.discovered : false,
                count: serverIng ? serverIng.count : 0,
  ...
  const exploreIngredients = async () => {
    ...
    try {
      // 상태 업데이트 준비 (에너지 소모 및 경험치 증가)
      const newGameState = {
        ...gameState,
        energy: gameState.energy - 50,
        // currentExp: gameState.currentExp + 20,
      };
      ...
      if (randomValue < config.foodRatio) {
        ...
        if (!selectedRecipe || !selectedRecipe.id) {
          throw new Error("선택한 레시피가 없습니다.");
        }

        // 미발견 식재료 필터링
        const undiscoveredIngredients = discoveredIngredients.items.filter(
          (item) => !item.discovered
        );

        // 레시피의 모든 식재료를 이미 발견한 경우
        if (undiscoveredIngredients.length === 0) {
          // 이미 발견한 식재료 중 필요 수량에 도달하지 않은 것들 필터링
          const neededIngredients = discoveredIngredients.items.filter(
            (item) => item.count < item.quantity
          );

          // 필요한 식재료가 없거나 레시피가 완성된 경우
          ...
          const randomIngredient =
            neededIngredients[
              Math.floor(Math.random() * neededIngredients.length)
            ];

          // 선택한 식재료 발견 처리
          const ingredientName = randomIngredient.name;
          ...
          // 발견한 식재료 상태 업데이트
          const updatedIngredients = discoveredIngredients.items.map((item) => {
          ...
          if (discoveryResult.isRecipeCompleted) {
            ...
            setModalConfig({
              isOpen: true,
              title: "식재료 획득",
              message: `${ingredientName} 1개를 획득했습니다.\n(현재: ${discoveryResult.newCount}/${randomIngredient.quantity})`,
              buttons: [
                { text: "확인", onClick: () => closeModal(), type: "primary" },
              ],
            });
          ...
          // 미발견 식재료 중 랜덤 선택
          const randomIngredient =
            undiscoveredIngredients[
              Math.floor(Math.random() * undiscoveredIngredients.length)
            ];
          ...
          setModalConfig({
            isOpen: true,
            title: "새로운 발견!",
            message: `새로운 식재료 '${ingredientName}'을(를) 발견했습니다!`,
            buttons: [
              { text: "확인", onClick: () => closeModal(), type: "primary" },
            ],
          });
        ...
        {loading ? (
          ...
          <>
            ...
            <LevelStatus
              ...
              food={gameState.food}
              ingredients={gameState.ingredientsCount}
              onFeed={feedCat}
